...
function getPresenceOp(lastPushedPresenceState, nextPresence) {
  ...
  if (lastPushedPresenceState && nextPresence) {
    const diff = (0, import_diff.diffRecord)(lastPushedPresenceState, nextPresence);
    if (!diff) return void 0;
    return [import_diff.RecordOpType.Patch, diff];
  }
  return void 0;
}
class TLSyncClient {
  ...
  constructor(config) {
    ...
    this.fpsScheduler = new import_utils.FpsScheduler(COLLABORATIVE_MODE_FPS);
    this.sendUnsentChanges = this.fpsScheduler.fpsThrottle(() => {
      ...
      const diff = this.unsentChanges.nextDiff ? (0, import_diff.getNetworkDiff)(this.unsentChanges.nextDiff) ?? void 0 : void 0;
  ...
  didReconnect(event) {
    ...
    (0, import_state.transact)(() => {
      ...
      this.store.mergeRemoteChanges(() => {
        ...
        const networkDiff = (0, import_diff.getNetworkDiff)(stashedChanges);
  ...
  push(change) {
    ...
    if (!this.unsentChanges.nextDiff) {
      this.unsentChanges.nextDiff = (0, import_utils.structuredClone)(change);
    } else {
  ...
  applyNetworkDiff(diff, runCallbacks) {
    ...
    for (const [id, op] of (0, import_utils.objectMapEntries)(diff)) {
      if (op[0] === import_diff.RecordOpType.Put) {
        ...
        if (existing && !(0, import_utils.isEqual)(existing, op[1])) {
          hasChanges = true;
      ...
      this.store.mergeRemoteChanges(() => {
        ...
        try {
          this.speculativeChanges = this.store.extractingChanges(() => {
            ...
            const diff = (0, import_diff.getNetworkDiff)(this.unsentChanges.nextDiff);
            if (!diff) return;
